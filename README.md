...
처음에는 Excel에 있는 네이버 카페 URL을 읽어 조회수, 댓글 수, 작성일을 수집하고 결과 Excel을 만드는 기능으로 시작했습니다. 실제 사용 중 필요한 기능이 늘어나면서 블로그 정리, 이미지 처리, 검사기, 설정, 자동 업데이트를 추가했습니다.
...
범용 RPA나 범용 크롤러를 목표로 만든 프로그램은 아닙니다. 현재 사용 중인 업무 흐름과 대상 사이트에 맞춰 구현되어 있습니다.
...
배포는 별도 `Monthly_Excel.Launcher`가 GitHub Releases의 최신 `manifest.json`을 확인하는 방식입니다. SHA-256을 비교해 필요한 파일을 갱신한 뒤 본 프로그램을 실행합니다.
