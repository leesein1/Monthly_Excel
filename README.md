...
여자친구 회사에서 반복되던 웹 조회와 Excel 정리 작업을 줄이기 위해 만든 WinForms 프로그램입니다.

처음에는 Excel에 있는 네이버 카페 URL을 읽어 조회수, 댓글 수, 작성일을 수집하고 결과 Excel을 만드는 기능으로 시작했습니다. 이후 회사에서 실제로 사용하면서 나온 수정사항과 요청을 반영해 블로그 정리, 이미지 처리, 검사기, 설정, 자동 업데이트까지 기능을 늘렸습니다.
...
이 작업을 줄이기 위해 Excel의 URL 목록을 읽어 필요한 정보를 수집하고 결과 파일을 생성하는 프로그램부터 만들었습니다.

프로그램은 실제 회사 업무에 사용됐고, 사용 과정에서 나온 요청을 기준으로 계속 수정했습니다. 창 크기와 화면 사용성 조정, 블로그 정리, 이미지 다운로드, 검사기, 자동 업데이트 같은 기능도 이 과정에서 추가됐습니다.
